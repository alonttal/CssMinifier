...
```bash
mvn test              # Run all 399 tests
mvn package -q -DskipTests  # Build JAR
```

## Architecture

All code is in `src/main/java/cssminifier/CssMinifier.java` (~814 lines).
...
- **Vendor fallback preservation**: `deduplicateBlock` checks for vendor prefixes (`-webkit-`, `-moz-`, etc.) and modern CSS functions (`calc()`, `var()`, etc.) in values before deduplicating.
